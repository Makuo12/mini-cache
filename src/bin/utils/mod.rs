...
impl Cache {
    ...
    pub async fn handle_cmd(&self, cmd: Command, memory: Arc<Mutex<Memory>>) -> CacheResult {
        match self {
            Self::Set => {
                if cmd.len() == 1 {
                    return self.set(cmd, memory).await;
                }
                CacheResult::Failure(String::from("Use set to store a single key an value pair.\nset key value"));
            }
            Self::HSet => {
                if cmd.len() % 2 == 0 && cmd.len() > 1 {
                    return self.set(cmd, memory).await;
                }
                CacheResult::Failure(String::from("Use set to store a single key an value pair.\nset key value"));
            }
    ...
    async fn get(&self, mut cmd: Command, memory: Arc<Mutex<Memory>>) -> CacheResult {
        ...
        memory.get(key_value)
    }
    async fn set(&self, cmd: Command, memory: Arc<Mutex<Memory>>) -> CacheResult {
        // key -> command\tkey
        // value -> value\"value\"value\n
        let mut memory = memory.lock().await;
        return memory.set(cmd.key, cmd.data, cmd.action);
    }
...
impl Command {
    pub fn new(size: usize, data: Vec<u8>) -> Result<Command, MainError> {
        ...
        return Ok(Command { data: values, key, action, reverse: String::new() })
    }
    fn len(&self) -> usize {
